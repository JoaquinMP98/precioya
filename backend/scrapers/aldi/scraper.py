...
_SEARCH_URL = f"{_BASE_URL}/es/buscar/"
_GRID_SELECTOR = ".product-tile, [class*='product-tile'], [class*='product-card']"
_TIMEOUT = 15_000
...
() => {
    ...
    return cards.map(card => {
        ...
        const nameEl = card.querySelector('.product-tile__description') ||
                       card.querySelector('[class*="product-tile__name"]') ||
                       card.querySelector('[class*="product-name"]') ||
                       card.querySelector('[class*="tile-name"]') ||
                       card.querySelector('h2, h3, p');
        const name = nameEl ? nameEl.textContent.trim() : '';

        const priceEl = card.querySelector('.product-tile__price') ||
                        card.querySelector('[class*="product-tile__price"]') ||
        ...
        const unitEl = card.querySelector('[class*="price-per-unit"]') ||
                       ...
                       card.querySelector('[class*="base-price"]');
        const price_per_unit = unitEl ? unitEl.textContent.trim() : null;

        const imgEl = card.querySelector('img[src*="aldi"]') ||
...
_EXTRACT_DETAIL_JS = """
() => {
    const nameEl = document.querySelector('[class*="product-detail__name"]') ||
                   document.querySelector('[class*="product-title"]') ||
                   document.querySelector('h1');
    const name = nameEl ? nameEl.textContent.trim() : '';

    const priceEl = document.querySelector('[class*="product-detail__price"]') ||
    ...
    const imgEl = document.querySelector('[class*="product-detail"] img') ||
                  document.querySelector('[class*="product-gallery"] img') ||
                  document.querySelector('img[src*="aldi"]');
...
class AldiScraper(BaseScraper):
    ...
    async def _do_search(self, query: str) -> list[ScrapedProduct]:
        async with async_playwright() as pw:
            ...
            try:
                ...
                logger.warning("Aldi search timed out for %r", query)
                return []
            finally:
                await context.close()
                await browser.close()
    ...
    async def get_product(self, url: str) -> ScrapedProduct | None:
        ...
        try:
            ...
            logger.warning("Aldi get_product failed for %s: %s", url, exc)
            return None
    ...
    async def _do_get_product(self, url: str) -> ScrapedProduct | None:
        async with async_playwright() as pw:
            ...
            try:
                ...
                await page.wait_for_selector(
                    "[class*='price-value'], [class*='product-detail__price'], [class*='price']",
                    state="attached",
                    timeout=self._timeout,
                )
                ...
                logger.warning("Aldi get_product timed out for %s", url)
                return None
            finally:
                await context.close()
                await browser.close()
